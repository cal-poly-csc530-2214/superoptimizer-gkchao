...
/* This is the signum function for which it is desired to find more efficient
code. */

int userfun(int x) {
   if (x > 0) return 1;        // return 1 on positive.
   else if (x < 0) return -1; // -1 on negative.
   else return 0;             // 0 on no sign (zero).
}
...
int trialx[] = {1, 0, -1, (int) MAXNEG, (int) MAXPOS, \
   (int) MAXNEG + 1, (int) MAXPOS - 1, 0x01234567, 0x79ABCDEF, -2, 2, -3, 3, \
   -64, 64, -5, -31415};
#if NARGS == 2
